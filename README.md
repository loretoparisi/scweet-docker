...
# Docker
This is the `Dockerfile` to build the `Scweet` image.

## Build [docker]
We build the Docker container
```
docker build -f Dockerfile -t scweet .
```

## Run [docker]
We run the Docker image with a attached `/src` volume for editable sources and a `/root` volumes in `$VOLUME` for files.
```
VOLUME=/myvolume docker run --rm -it -d -v `pwd`:/app -v $VOLUME:/root --name scweet-test scweet
docker exec -it scweet-test bash
```
